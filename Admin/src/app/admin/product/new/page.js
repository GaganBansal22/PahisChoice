...
const Page = () => {
  ...
  async function handleSubmit(evt) {
    ...
    try {
      setShowMessage({ loading: true, message: "Register new Product...Please wait" })
      let res = await axios.post(`/api/admin/product/new`, formData,
        { headers: { "Content-Type": "multipart/form-data" }, withCredentials: true });
      setShowMessage(false)
      if (res.data.status == "success") {
        setShowMessage({ success: true, message: "Registered Successfully..." })
        setTimeout(() => {
          router.replace(`/product/${res.data.pid}`)
        }, 1000);
      }
      else
        ...
        <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
          <form onSubmit={handleSubmit} className="max-w-md mx-auto FormDetails">
            ...
            <div className='relative z-0 w-full mb-5 group'>
              ...
              <input className="block w-full text-sm text-gray-900 border border-gray-300 cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" aria-describedby="images" name='image' id="image" type="file" multiple required />
            </div>
