...
function Page() {
    ...
    useEffect(() => {
        if (!loginDetails.user)
            return router.replace(`/login?returnTo=/account/myorders/${oid}`)
    }, [loginDetails.user, router])
    useEffect(() => {
        (async () => {
            try {
                let res = await axios.get(`/api/account/myorders/${oid}`, { withCredentials: true })
    ...
    return (
        <>
            ...
            {orderDetails &&
                <div className='md:mt-[8rem] mt-[4rem] mb-[6rem] md:border-[1px] border-slate-200 md:rounded-md md:mx-56 p-5'>
                    <div>
