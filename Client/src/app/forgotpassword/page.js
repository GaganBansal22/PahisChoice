...
const Page = () => {
    ...
    async function sendOTP() {
        try {
            ...
            let res = await axios.post(`/api/api/send-otp`, { to: formDetails.email, text: message, reset: true })
    ...
    async function handleSubmit(evt) {
        ...
        try {
            ...
            let res = await axios.post(`/api/account/forgotpassword`, formDetails,{withCredentials:true})
            if (res.data.status == "success") {
                ...
                setShowMessage({error:true,message:"Token expired!"})
                setTimeout(() => {
                    setShowMessage(false)
                    setFormDetails({ email: "", otp: "", password1: "", password2: "" })
                    setOtpSent(false)
